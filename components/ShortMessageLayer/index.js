...
import generateTpl from './generateTpl.js';
import './index.scss';
...
class ShortMessageLayer {
    ...
    render() {
        ...
        $(renderNode).append(tpl).addClass('overflow-hidden');
    ...
    checkGetCodeState() {
        ...
        if (currentInterval < minSendRequestInterval) {
            this.disableGetCodeBtn(Math.ceil((minSendRequestInterval - currentInterval) / 1000));
        }
    }
    ...
    sendCode(code) {
        ...
        $.ajax({
            ...
            complete: () => {
                $('.J_shortMessageSubmit').length && $('.J_shortMessageSubmit').text(sendCodeText);
            }
        })
    }
    ...
    dispose() {
        ...
        $(renderNode).removeClass('overflow-hidden');
    }
}
